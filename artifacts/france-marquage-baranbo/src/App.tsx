...
const siteTranslations: Record<Language, SiteCopy> = {
  EN: {
    ...
    fleet: {
      ...
      description: 'Every machine in our fleet is chosen for precision, reliability, and minimal disruption — keeping your site moving safely and efficiently.',
      contact: 'Explore our fleet',
  ...
  FR: {
    ...
    fleet: {
      ...
      titleSecond: 'la journée de travail.',
      description: 'Chaque machine de notre flotte est choisie pour sa précision, sa fiabilité et son impact minimal — pour un chantier sûr et efficace.',
      contact: 'Découvrez notre flotte',
  ...
  AR: {
    ...
    fleet: {
      ...
      description: 'كل آلة في أسطولنا مختارة بعناية لدقتها وموثوقيتها وأقل تأثير على سير العمل، لتبقى مواقعكم آمنة وفعّالة.',
      contact: 'استكشف أسطولنا',
          ...
          <div className="mx-auto max-w-[1380px]">
            ...
            <div className="mt-16 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {services.map((service, index) => {
                ...
                const isImageCard = service.number === '02' || service.number === '03' || service.number === '04' || service.number === '05' || service.number === '06';
                const cardBackgroundImage = service.number === '01'
                  ...
                  : service.number === '02'
                    ...
                    : service.number === '06'
                      ? '/images/services/mechanical-street-sweeping-card.jpg'
                    : null;
                return (
                  <article
                    key={service.number}
                    className={`service-card reveal reveal-delay-${(index % 3) + 1} ${service.number === '05' || service.number === '06' ? 'relative' : ''} flex min-h-[390px] cursor-pointer flex-col justify-between p-7 active:scale-[.99] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#d9673f] ${isImageCard ? 'bg-[#F59E0B] text-[#F8FAFC]' : isDark ? 'bg-[#1E293B] text-[#F8FAFC]' : isYellow ? 'bg-[#F59E0B] text-[#1E293B]' : 'bg-[#F8FAFC] text-[#1E293B]'}`}
                    role="button"
                    tabIndex={0}
                    ...
                    <div className="relative z-10">
                      ...
                      <a
                        ...
                        onKeyDown={(event) => event.stopPropagation()}
                         className={`group mt-7 inline-flex w-fit items-center gap-3 border-b pb-2 font-mono-site text-[10px] font-bold uppercase tracking-[.12em] transition-colors focus-visible:outline-2 focus-visible:outline-offset-4 ${service.number === '01' ? 'relative isolate overflow-hidden' : ''} ${isDark || isImageCard ? 'border-[#f3c742] text-[#f3c742] hover:border-[#f6f1e6] hover:text-[#f6f1e6] focus-visible:outline-[#f3c742]' : isYellow ? 'border-[#1E293B] text-[#1E293B] hover:border-[#f8fafc] hover:text-[#f8fafc] focus-visible:outline-[#1E293B]' : 'border-[#d9673f] text-[#d9673f] hover:border-[#1E293B] hover:text-[#1E293B] focus-visible:outline-[#d9673f]'}`}
